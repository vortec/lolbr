...
var _internal_handlers = {
    ...
    IDLE: function(io, socket, input) {
        var idle = input['idle'];
        if (idle) {
            socket.user.idle = true;
            socket.room.broadcast('IDLE', {nickname: socket.session.nickname, idle: true});
        } else {
            socket.user.idle = false;
            socket.room.broadcast('IDLE', {nickname: socket.session.nickname, idle: false});
        }
    },
    ...
    MESSAGE: function(io, socket, input) {
        ...
        if (!input_text) {
            socket.emit('MISSING_PARAMETER', {parameter: 'text'});
            return;
        }
    ...
    USER_LIST: function(io, socket, input) {
        var nicknames = socket.room.getNicknames();
        socket.user.push(socket.room, 'USER_LIST', {nicknames: nicknames});

        for (var i in nicknames) {
            var nickname = nicknames[i];
            socket.emit('IDLE', {nickname: nickname, idle: users.get(nickname).idle});
        }
    }
}
